...
resource "aws_security_group" "cluster" {
  ...
  dynamic "ingress" {
    for_each = length(var.cluster_api_access_cidrs) > 0 ? [1] : []
    content {
      description = "Allow Kubernetes API from trusted admin CIDRs"
      from_port   = 443
      to_port     = 443
      protocol    = "tcp"
...
resource "aws_eks_cluster" "this" {
  ...
  vpc_config {
    ...
    endpoint_public_access  = length(var.cluster_api_access_cidrs) > 0
